...
class _CompleteProfileProgressState
  ...
  Widget build(BuildContext context) {
    ...
    final inCompletedItems = [];

    if (widget.user?.profile_img_url != null) {
      completedItems.add(_fieldStatus(
          title: context.l10n.profile_complete_add_profile_picture,
          isCompleted: true));
    } else {
      inCompletedItems.add(_fieldStatus(
          title: context.l10n.profile_complete_add_profile_picture,
          isCompleted: false));
    }
    ...
    if (widget.user?.dob != null && widget.user?.gender != null) {
      completedItems.add(_fieldStatus(
          title: context.l10n.profile_complete_add_personal_details,
          isCompleted: true));
    } else {
      inCompletedItems.add(_fieldStatus(
          title: context.l10n.profile_complete_add_personal_details,
    ...
    if (widget.user?.player_role != null &&
      ...
      completedItems.add(_fieldStatus(
          title: context.l10n.profile_complete_add_playing_style,
          isCompleted: true));
    } else {
      inCompletedItems.add(_fieldStatus(
          title: context.l10n.profile_complete_add_playing_style,
  ...
  Widget _fieldStatus({
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Icon(
            isCompleted ? Icons.check_circle : Icons.radio_button_off_rounded,
            color: isCompleted
