...
class MatchScheduler {
  ...
  MatchScheduler(this.teams, List<MatchModel> matches) {
  ...
  /// Schedules knockout matches in a tournament format.
  GroupedMatchMap scheduleKnockOutMatches() {
    final GroupedMatchMap additionalScheduledMatches = {};
    final teamPool = List.of(teams);
    int currentRound = 1;

    // Initialize the first round group if it doesn’t exist
    additionalScheduledMatches[MatchGroup.round] = {};
    additionalScheduledMatches[MatchGroup.round]![currentRound] = [];

    while (teamPool.length > 1) {
      // Determine the MatchGroup based on remaining teams
      MatchGroup currentGroup;
      if (teamPool.length > 4) {
        currentGroup = MatchGroup.round;
      } else if (teamPool.length == 4) {
        currentGroup = MatchGroup.quarterfinal;
        currentRound = 1;
      } else if (teamPool.length == 2) {
    ...
    // Remove teams already scheduled for this round
    teamPool.removeWhere((team) => scheduledMatches[currentGroup]![currentRound]!
        .any((match) => match.teams.map((e) => e.team.id).contains(team.id)));
    ...
    // Add match to scheduledMatches and additionalScheduledMatches
    scheduledMatches[currentGroup]![currentRound]!.add(match);
    additionalScheduledMatches[currentGroup]![currentRound]!.add(match);
    }
    }

    // Collect winners to move to the next round
    final finishedMatchesWinners = scheduledMatches[currentGroup]![currentRound]!
        .where((match) => match.matchResult != null)
        .map((match) => match.teams
        .firstWhere((element) => element.team.id == match.matchResult!.teamId)
        .team)
        .toList();
